...
from fake import FakeHover, FakeTrack, FakeTurn, Swarm, Formation, MultiHover, FormationBall, FormationBallForward, PID
import time
...
def main(cfg):
    ...
    ckpt_name = "rl_data/turn/Turn_smooth_loadwosim2real.pt"
    data_frame = []

    debug = False
    swarm = Swarm(cfg, test=debug)
    base_env = FakeTurn(cfg, connection=True, swarm=swarm)
    ...
    swarm.end_program()
    torch.save(data_frame, "rl_data/debug.pt")
